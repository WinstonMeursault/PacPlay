...
int packetInitData(Packet *packet, MessageType msgType, uint32_t seqID,
    ...
    if (packet->payload != NULL) {
        LOG_ERROR("packetInitData: payload is not NULL; must be cleared first");
        return PROTOCOL_FAIL;
    }
    if (dataLen > DATA_MAX_PAYLOAD_LEN) {
        LOG_ERROR("packetInitData: dataLen %zu exceeds DATA_MAX_PAYLOAD_LEN %u",
                  dataLen, DATA_MAX_PAYLOAD_LEN);
        return PROTOCOL_FAIL;
    }
    if (dataLen > 0 && data == NULL) {
        LOG_ERROR("packetInitData: dataLen %zu > 0 but data is NULL", dataLen);
        return PROTOCOL_FAIL;
    }
    ...
    if (dataLen > 0) {
        packet->payload = malloc(dataLen);
        if (packet->payload == NULL) {
            LOG_ERROR("packetInitData: failed to allocate payload: %s (%d)",
                      strerror(errno), errno);
            return PROTOCOL_FAIL;
        }
        memcpy(packet->payload, data, dataLen);
...
int packetSendEncryptedData(SocketFD fd, MessageType mt, uint32_t *seqID,
    ...
    if (packetInitData(&pkt, mt, *seqID, PlaintextPacket, data, dataLen) !=
        PROTOCOL_SUCC) {
        return PROTOCOL_FAIL;
    }
