...
echo "=== Packaging ==="
TMPDIR="$(mktemp -d /tmp/pacpack.XXXXXX)"
trap 'rm -rf "$TMPDIR"' EXIT

mkdir -p "$TMPDIR/server/linux"
cp "$PROJECT_ROOT/bin/pacman_server.so" "$TMPDIR/server/linux/pacmanServer.so"
tar czf "$TMPDIR/server.tar.gz" -C "$TMPDIR/server" linux/

mkdir -p "$TMPDIR/client/linux"
cp "$PROJECT_ROOT/bin/pacman_client.so" "$TMPDIR/client/linux/pacmanClient.so"
